...
router.get("/export.xlsx", requirePermission("question_areas:read"), async (req, res) => {
  ...
  const rows = result.rows.map((row) => ({
    ...
    "Title Evidence": formatExportBoolean(row.exists_in_legal_layer),
    "Property Asset": formatExportBoolean(row.exists_in_management_layer),
    "Tax Bill Received": formatExportBoolean(row.exists_in_client_tabular_bill_data),
